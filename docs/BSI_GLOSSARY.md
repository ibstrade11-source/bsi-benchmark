...
It exists to help the judge understand BSI-specific abbreviations, constructs, layers, and
dimension names that may appear in a BSI analysis.

**It is not an evaluation rubric.** It does not instruct the judge that BSI is better than RAW,
that any BSI construct is correct, or that a BSI conclusion should be accepted.

The judge must continue to evaluate RAW and BSI using independently selected criteria.
...
### REIG — Recursive Epistemic Integrity Gap
...
The following principle is mandatory for benchmark interpretation:

**If an abbreviation or technical term is not defined in this glossary and is not defined in the
analysis itself, the judge must not invent its meaning.**

For example, terms such as `HHI`, `BHI`, or `DDC` should not be assigned BSI-specific meanings
unless a definition is supplied by the analysis or an authoritative domain source.

The same rule applies to any unfamiliar acronym.

---

## 13. Judge-Neutrality Boundary

This glossary is supplied only to reduce **semantic ambiguity**.

It must not be used to:

1. force the judge to use BSI's own D1–D7 criteria;
2. assume that BSI is superior to RAW;
3. treat BSI terminology as evidence of analytical quality;
4. treat the existence of an EIG/ECC/REIG finding as proof that the underlying claim is false;
5. treat a high BSI score as proof of truth;
6. treat a low BSI score as proof of falsity;
7. infer missing meanings for undefined abbreviations;
