...
It explains BSI-specific abbreviations, constructs, layers, and dimension names appearing in BSI
analyses. It is **not an evaluation rubric**: it does not assert that BSI is better than RAW,
that any BSI construct is correct, or that any BSI conclusion should be accepted. The judge must
evaluate RAW and BSI using independently selected criteria.
...
A recursive audit of the **analysis itself**, not an EIG analysis of the original target artifact.
REIG asks whether the analyst's output complies with the epistemic standards applied to its target.

**Distinction:**
- **EIG →** evaluates epistemic gaps in the target.
- **REIG →** audits the analysis produced about that target.

REIG operates on the draft analysis/output, not directly on the source article. Typical checks include
causal, generalization, measurement, and framing compliance. A REIG finding alone does not mean the
entire analysis is false or must be rejected.
...
**Mandatory:** If an abbreviation or technical term is undefined in this glossary and in the analysis,
the judge must not invent its meaning. Terms such as `HHI`, `BHI`, or `DDC` must not receive
BSI-specific meanings unless defined by the analysis or an authoritative domain source. The same
rule applies to every unfamiliar acronym.
...
1. force use of BSI's D1–D7 criteria;
2. assume BSI is superior to RAW;
3. treat BSI terminology as evidence of analytical quality;
4. treat an EIG/ECC/REIG finding as proof that an underlying claim is false;
5. treat a high BSI score as proof of truth;
6. treat a low BSI score as proof of falsity;
7. infer meanings for undefined abbreviations;
8. replace independent human validation of analytical quality.

The benchmark judge remains responsible for selecting and applying independent evaluation criteria.
...
benchmark context: `docs/_reference/GLOSSARY.md`, `Ontology/BIO_v1.0.md`,
...
`BSI_Calculation_Formula_Hybrid_v3x.md`, and `MASTER_PROMPT_BSI_v3.4.2.md`.

Where definitions overlap, the more specific module documentation clarifies the operational meaning.
